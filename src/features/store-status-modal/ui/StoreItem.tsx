...
export default function StoreItem({ store, newBadgeOpenMonth }: StoreItemProps) {
  const openDate = store.open_date?.trim() ?? '';
  const { label, upcoming, isNewOpen } = getOpenStatus(openDate, newBadgeOpenMonth);
  const cardClass = isNewOpen
